...
import pygame
# Główny moduł Pythona z którego będziemy korzystać (zaskakujące).
# Pozwala w łatwy sposób wyświetlać grafikę 2D.
import random  # Moduł zawierający generatory liczb pseudolosowych.


pygame.init()
"""Z pewnych względów technicznych musimy zaincjalizować PyGame
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
"""Każdy dobry pomysł na usprawnienie tej gry, będzie pozytywnie wpływał
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
"""W tej wersji gry sterujemy strzałkami w lewo i w prawo."""
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
"""Skrypt ,,Gra_bazowa_01.py'' zawiera minimalną ilość kodu który pozwala
...
Pliki Gra_w_uniki_03-04 ilustrują, jak zrobić w Pythonie prostą grę,
...
"""PyGame, z nieznanych mi powodów, nie definiuje palety domyślnych kolorów,
...
"""Tworzymy okienko w którym będzie wyświetlana gra. Jako argument podajemy
...
"""Ładujemy do gry obrazek przedstawiający ,,bohatera'', który znajduje się
...
def hero(x, y):
    ...
    PyGame używa następującego układu współrzędnych. Położenie (0, 0)
...
def things(thing_x, thing_y, thing_width, thing_height, color):
    """Funkcja ta rysuje ,,thing'' jako prostokąt o szerokości
...
def text_objects(text, font, color=black):
    """Zwraca teksturę z napisanym na niej tekstem ,,text'' oraz prostokąt
...
def message_display(text):
    """Funkcja która wyświetla ,,text'' na ekranie."""
...
def crash_info():
    """Funkcja która uruchamia się gdy przegramy i wyświetla
...
delta_space = 5
"""Pomocnicza zmienna do kontroli prędkości przesuwania się ,,bohatera''.
...
"""Maksymalne położenie w prawo w jakim może się znaleźć ,,bohater'' zanim
...
def game_loop():
    ...
    """Proszę Państwa, nie jestem game developerem, za to przez wiele lat
    ...
    delta_x = 0
    """Zmienna do przesuwania ,,bohater'' w kierunku osi x. Pozycja
    ...
    """Losujemy położenie ,,thing'' na osi x. Gdyby za każdym razem
    pojawiały się w tym samym miejscu, gra byłaby nudna, dlatego
    wykorzystujemy generator liczb pseudolosowych z modułu random.
    Funkcja z tego modułu randrange generuje pseudolosową liczbę z zakresu
    od 0 do (display_width - thing_width)."""

    thing_start_y = -600
    """Na początku gry umieszczamy ,,thing'' trochę powyżej ekranu,
    ...
    # Kluczowa pętla gry.
    while play_game:
        """Jak napisano wyżej, gra to jedna wielka pętla while,
        ...
        for event in pygame.event.get():
            """PyGame cały czas zbiera ogromną ilość danych o działaniu
            ...
            Możemy wypisać listę zdarzeń z danej klatki w powłoce Pythona
            ...
            if event.type == pygame.QUIT:
                """Event jest obiektem, posiada jak swój atrybut swój typ.
            ...
            if event.type == pygame.KEYDOWN:
                """Sprawdzamy czy jakiś klawisz został WCIŚNIĘTY. Jeżeli
                ...
                if event.key == pygame.K_LEFT:
                    """Sprawdzamy który klawisz został wciśnięty.
            ...
            if event.type == pygame.KEYUP:
                """Sprawdzamy czy puściliśmy jakiś klawisz."""
                if (event.key == pygame.K_LEFT) \
                   or (event.key == pygame.K_RIGHT):
                    """Jeśli puścimy strzałkę w lewo lub prawo, to musimy
        ...
        """Fizyka w grach komputerowych polega na tym, żeby obiekty w grze
        ...
        """W tym momencie taki kod jest zupełnie wystarczający:
        x_(n + 1) = x_n + delta_x,
        y_(n + 1) = y_n + delta_y.
        ...
        if x <= 0:
            """Skoro x <= od zera, to znaczy, że albo ,,bohater'' jest
        ...
        """Jest kilka powodów dla których należy najpierw wszystko policzyć,
        ...
        """W tym momencie tło i ,,bohater'' są ,,narysowani'' W PAMIĘCI
        komputera, ale nie są wyświetlani na ekranie komputera. Dopiero
        poniższa komenda to robi."""

        ##############################
        """Sprawdzamy, czy ,,thing'' wyszło poza ekran. Jeśli tak,
        ...
        """Sprawdzamy, czy nie doszło do kolizji z ,,thing''.
        ...
        """Skoro narysowaliśmy już ,,thing''  gdzie chcieliśmy
...
pygame.quit()
"""PyGame tak jak został włączony (zainicjalizowany), musi zostać
